...
This roadmap gives a high-level view of where the Digital Infrastructure Observatory is today and where it is heading. It is a living document and will evolve as the project grows.
...
- An AI-assisted data pipeline with a human review gate that keeps both the cable and policy datasets current.
...
- **Scheduled runs** for both the cable and policy pipelines.
...
- **Security and access hardening** for public use (including row-level security on the database before the repository and platform are opened up).
